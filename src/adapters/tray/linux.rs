...
/// Shared state accessible from both the D-Bus interface and the command loop.
pub struct TraySharedState {
    tooltip_title: String,
    tooltip_body: String,
}

/// D-Bus StatusNotifierItem interface implementation.
///
/// Registered at `/org/bit7z/TrayIcon` on the session bus.
/// Handles `Activate` (left-click → restore window), `SecondaryActivate` (middle-click),
/// and `ContextMenu` (right-click → show Open/Exit in the host's menu).
pub struct TrayIcon {
...
impl TrayIcon {
    ...
    fn secondary_activate(&self, _x: i32, _y: i32) {
        // Middle-click: treat as right-click for now
...
/// Run the Linux tray icon on the current thread.
///
/// Connects to the D-Bus session bus, registers a StatusNotifierItem,
/// and enters a command-processing loop.  D-Bus method calls are handled
/// by zbus's internal message pump — this thread only needs to poll for
/// `TrayCommand` messages from the application.
    ...
    loop {
        match cmd_rx.try_recv() {
            ...
            Ok(TrayCommand::UpdateProgress { message, percent }) => {
                ...
                // Notify tray hosts to re-read the ToolTip property.
                // Ignore errors — some hosts poll anyway.
...
/// Emit `NewToolTip` signal to notify the tray host.
...
/// Fallback loop when D-Bus is unavailable — polls for exit command only.
