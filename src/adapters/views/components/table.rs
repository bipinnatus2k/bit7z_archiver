...
/// Column definition for a sortable table header.
/// `width: None` means flex_1 (fills remaining space).
pub struct Column {
    pub label: SharedString,
    /// None = flex_1, Some(pixels) = fixed width.
...
/// Build a sortable table header row with sort indicators.
/// Follows gpui-component composable builder patterns.
pub fn sortable_header(
    ...
    for (i, col) in columns.iter().enumerate() {
        ...
        if let Some(w) = col.width {
            ...
            // flex_1 + basis(0) + min_w(0) ensures consistent sizing
