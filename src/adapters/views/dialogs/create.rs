...
impl CreateArchiveDialog {
    ...
    //     self.password_state = Some(cx.new(|cx| {
    //         InputState::new(window, cx)
    //             .placeholder("Optional")
    //             .masked(true)
    //     }));
    // }
    // if self.password_confirm_state.is_none() {
    //     self.password_confirm_state = Some(cx.new(|cx| {
    //         InputState::new(window, cx)
    //             .placeholder("Confirm password")
    //             .masked(true)
    //     }));
    ...
    //
    //     self.format_select = Some(state);
    // }
    // if self.level_select.is_none() {
    //
    //     self.level_select = Some(state);
    // }
    // }

    // fn toggle_password_visibility(&mut self, window: &mut Window, cx: &mut Context<Self>) {
    //     self.show_password = !self.show_password;
    //     if let Some(ref state) = self.password_state {
    //         state.update(cx, |state, cx| {
    //             state.set_masked(!self.show_password, window, cx);
    //         });
    //     }
    //     if let Some(ref state) = self.password_confirm_state {
    //         state.update(cx, |state, cx| {
    //             state.set_masked(!self.show_password, window, cx);
    //         });
    //     }
...
impl Render for CreateArchiveDialog {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        // let selected = formats.iter().last();
                    ...
                    .child(
                        Button::new("add-files").icon(IconName::Plus).label("Add Files")
                            .on_click(cx.listener(|this: &mut CreateArchiveDialog, _event: &ClickEvent, _window: &mut Window, cx| {
                                if let Some(path) = crate::adapters::platform::pick_files() {
