...
impl TableDelegate for FileListTableDelegate {
    ...
    fn perform_sort(
            ...
            let ord = match key.as_str() {
                ...
                "ratio" => {
                    let ra = if a.size == 0 {
                        ...
                        1.0 - a.compressed_size as f64 / a.size as f64
                    };
                    let rb = if b.size == 0 {
                        0.0
                    } else {
                        1.0 - b.compressed_size as f64 / b.size as f64
                    };
        ...
        if let Some(fl) = self.file_list.upgrade() {
            fl.update(_cx, |_, cx| {
                cx.emit(FileListIntent::SortByColumn(col_ix as u32, ascending))
            });
        }
    }
...
impl ArchiveFileList {
    pub fn new(window: &mut Window, cx: &mut Context<Self>) -> Self {
        ...
        cx.subscribe_in(
            &table_state,
            window,
            |view, _table, event, _window, cx| match event {
...
impl Render for ArchiveFileList {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        match &self.status {
            ...
            ViewStatus::Loading => base.child(loading_view(cx)),
            ViewStatus::Error(msg) => base.child(error_view(cx, msg)),
            ViewStatus::Ready => {
                ...
                container = container.child(
                    Breadcrumb::new()
                        ...
                        .child(BreadcrumbItem::new(" \u{2190} ").on_click(
                            move |_: &ClickEvent, _: &mut Window, cx: &mut App| {
                                h.update(cx, |_, cx| cx.emit(FileListIntent::NavigateUp));
                            },
                        ))
                        .child(BreadcrumbItem::new(path_str)),
                );
