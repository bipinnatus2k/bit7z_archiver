...
use gpui::{AnyElement, App, Background, ClickEvent, Corners, Div, Edges, ElementId, Hsla, InteractiveElement, Interactivity, IntoElement, MouseButton, ParentElement, Pixels, RenderOnce, SharedString, Stateful, StatefulInteractiveElement as _, StyleRefinement, Styled, Window, div, prelude::FluentBuilder as _, relative, transparent_white};
use gpui_component::menu::{DropdownMenu};
use crate::toolbar::button_icon::ToolbarButtonIcon;

/// Direction of icon relative to label in a toolbar button.
///
/// Toolbar buttons default to `Vertical` (icon above label).
#[derive(Clone, Copy, PartialEq, Eq, Default, Debug)]
pub enum IconLabelDirection {
    /// Icon above label.
    #[default]
    Vertical,
    /// Icon beside label.
...
impl From<Pixels> for ToolbarButtonRounded {
    fn from(px: Pixels) -> Self {
        ToolbarButtonRounded::Size(px)
    }
}

#[derive(Clone, Copy, PartialEq, Eq, Debug)]
pub struct ToolbarButtonCustomVariant {
    color: Hsla,
    foreground: Hsla,
    shadow: bool,
    hover: Hsla,
...
pub trait ButtonVariants: Sized {
    fn with_variant(self, variant: ToolbarButtonVariant) -> Self;

    /// With the primary style for the Button.
    fn primary(self) -> Self {
        self.with_variant(ToolbarButtonVariant::Primary)
    }

    /// With the secondary style for the Button.
    fn secondary(self) -> Self {
        self.with_variant(ToolbarButtonVariant::Secondary)
    }

    /// With the ghost style for the Button.
    fn ghost(self) -> Self {
        self.with_variant(ToolbarButtonVariant::Ghost)
    }

    /// With the custom style for the Button.
    fn custom(self, style: ToolbarButtonCustomVariant) -> Self {
        self.with_variant(ToolbarButtonVariant::Custom(style))
    }
}

impl ToolbarButtonCustomVariant {
...
pub enum ToolbarButtonVariant {
    ...
    Secondary,
    Ghost,
    Custom(ToolbarButtonCustomVariant),
}
...
impl From<ToolbarButton> for AnyElement {
    fn from(button: ToolbarButton) -> Self {
        button.into_any_element()
    }
}

impl DropdownMenu for ToolbarButton {}
...
impl ToolbarButton {
    pub fn new(id: impl Into<ElementId>) -> Self {
        ...
        Self {
            ...
            size: Size::Medium,
            direction: IconLabelDirection::default(),
            tooltip: None,
    ...
    pub fn icon(mut self, icon: impl Into<ToolbarButtonIcon>) -> Self {
    ...
    /// Set the direction of icon relative to label.
    ///
    /// Toolbar buttons default to `Vertical` (icon above label).
    /// Set to `Horizontal` for icon beside label.
    pub fn with_direction(mut self, direction: IconLabelDirection) -> Self {
...
impl Sizable for ToolbarButton {
...
impl ButtonVariants for ToolbarButton {
...
impl RenderOnce for ToolbarButton {
    fn render(self, window: &mut Window, cx: &mut App) -> impl IntoElement {
        let style: ToolbarButtonVariant = self.variant;
        ...
        let focus_handle = window
            ...
            .read(cx)
            .clone();
        let _is_focused = focus_handle.is_focused(window);

        let rounding = match self.rounded {
            ToolbarButtonRounded::Small => cx.theme().radius * 0.5,
            ToolbarButtonRounded::Medium => cx.theme().radius,
            ToolbarButtonRounded::Large => cx.theme().radius * 2.0,
            ToolbarButtonRounded::Size(px) => px,
            ToolbarButtonRounded::None => Pixels::ZERO,
        };
        ...
        self.base
            ...
            .when_else(self.label.is_none() && self.children.is_empty(), |this| {
                // Icon Button
                match self.size {
            ...
            .child({
                let is_vertical = self.direction == IconLabelDirection::Vertical;
                (if is_vertical { v_flex() as Div } else { h_flex() as Div })
                    .id("label")
                    .size_full()
                    .items_center()
                    .justify_center()
...
impl ToolbarButtonVariant {
    fn outline_background(&self, state: ButtonStyleState, cx: &mut App) -> Background {
        match (self, state) {
            ...
            (Self::Ghost, _) => cx.theme().transparent.into(),
    ...
    fn bg_color(&self, outline: bool, cx: &mut App) -> Background {
        ...
        match self {
            ...
            Self::Ghost => cx.theme().transparent.into(),
    ...
    fn shadow(&self, outline: bool, _: &App) -> bool {
        match self {
            Self::Default => true,
            Self::Primary | Self::Secondary => outline,
    ...
    fn selected(&self, outline: bool, cx: &mut App) -> ButtonVariantStyle {
        ...
        let border = self.border_color(outline, cx);
        let fg = self.text_color(false, cx);
        // let underline = self.underline(cx);
        let shadow = self.shadow(outline, cx);
    ...
    fn disabled(&self, outline: bool, cx: &mut App) -> ButtonVariantStyle {
        let bg = match self {
            Self::Default | Self::Ghost => cx.theme().transparent.into(),
        ...
        let (bg, border) = if outline {
            ...
            let border = match self {
                ...
                Self::Default | Self::Ghost => cx.theme().transparent,
            };
            (bg, border)
        };
...
mod tests {
    ...
    fn test_button_builder(_cx: &mut gpui::TestAppContext) {
        let button = ToolbarButton::new("complex-button")
            .label("Save Changes")
            .primary()
            .outline()
            .large()
        ...
        assert_eq!(button.variant, ToolbarButtonVariant::Primary);
        ...
        assert!(matches!(button.rounded, ToolbarButtonRounded::Medium));
        assert_eq!(button.direction, IconLabelDirection::Vertical);
    }

    #[gpui::test]
    fn test_button_clickable_logic(_cx: &mut gpui::TestAppContext) {
        // Button with click handler should be clickable
        let clickable = ToolbarButton::new("test").on_click(|_, _, _| {});
        assert!(clickable.clickable());

        // Disabled button should not be clickable
        let disabled = ToolbarButton::new("test").disabled(true).on_click(|_, _, _| {});
        assert!(!disabled.clickable());

        // Loading button should not be clickable
        let loading = ToolbarButton::new("test").loading(true).on_click(|_, _, _| {});
    ...
    fn test_button_variant_methods(_cx: &mut gpui::TestAppContext) {
        ...
        assert!(ToolbarButtonVariant::Ghost.is_ghost());
    }


    #[gpui::test]
    fn test_button_direction(_cx: &mut gpui::TestAppContext) {
        let btn = ToolbarButton::new("dir-test")
            .label("Test")
            .with_direction(IconLabelDirection::Horizontal);
        assert_eq!(btn.direction, IconLabelDirection::Horizontal);

        let btn2 = ToolbarButton::new("dir-test2")
            .label("Test");
        assert_eq!(btn2.direction, IconLabelDirection::Vertical);
    }
    ...
    fn test_primary_button_uses_gradient_background_tokens(cx: &mut gpui::TestAppContext) {
        ...
        window.update(|_, cx| {
            let config = serde_json::from_value::<gpui_component::ThemeConfig>(serde_json::json!({
                "name": "Gradient",
                "mode": "light",
                "colors": {
            ...
            assert_eq!(
                ToolbarButtonVariant::Primary.normal(false, cx).bg,
                cx.theme().tokens.button_primary.into()
            );
            assert_eq!(
                ToolbarButtonVariant::Primary.hovered(false, cx).bg,
                cx.theme().tokens.button_primary_hover.into()
            );
            assert_eq!(
                ToolbarButtonVariant::Primary.active(false, cx).bg,
                cx.theme().tokens.button_primary_active.into()
            );
        });
    }
    ...
    fn test_outline_primary_keeps_original_depth(cx: &mut gpui::TestAppContext) {
        ...
        window.update(|_, cx| {
            ...
            assert_eq!(
                ToolbarButtonVariant::Primary.normal(true, cx).bg,
                cx.theme().tokens.primary.background.opacity(0.1)
            );
            assert_eq!(
                ToolbarButtonVariant::Primary.hovered(true, cx).bg,
                cx.theme().tokens.primary_hover.background.opacity(0.2)
            );
            assert_eq!(
                ToolbarButtonVariant::Primary.active(true, cx).bg,
                cx.theme().tokens.primary_active.background.opacity(0.4)
            );
        });
    }
    ...
    fn test_disabled_outline_buttons_keep_semantic_backgrounds(cx: &mut gpui::TestAppContext) {
        ...
        window.update(|_, cx| {
            ...
            assert_eq!(
                ToolbarButtonVariant::Primary.disabled(true, cx).bg,
                cx.theme()
                    .tokens
                    .primary
                    .background
