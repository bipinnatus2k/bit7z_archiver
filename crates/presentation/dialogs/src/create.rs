...
use bit7z_domain::archive::{
    ArchiveFormat, CreateFileItem, EncryptionConfig, EncryptionMethod, Password,
};
...
impl CreateArchiveDialog {
    pub fn new(window: &mut Window, cx: &mut Context<Self>, files: Vec<CreateFileItem>) -> Self {
        ...
        let format_select = cx
            .new(|cx| SelectState::new(writable_formats(), Some(IndexPath::default()), window, cx));
        ...
        v_flex()
            ...
            .child(
                div()
                    ...
                    .children(if self.file_list.is_empty() {
                        ...
                        self.file_list
                            .iter()
                            .map(|f| div().px_2().py_1().child(f.display_name()).into_any())
                            .collect()
                    }),
            )
            .child(
                h_flex()
                                 ...
                                 cx| {
                                    if let Some(path) = bit7z_infra_platform::pick_files() {
                                        let _ = path.iter().map(|i| {
                                            this.file_list.push(
                                                bit7z_domain::archive::CreateFileItem {
                                                    path: i.clone(),
                                                    is_directory: false,
                                        ...
                                        if this.destination.is_empty() {
                                            let stem = path
                                                ...
                                                .unwrap_or_default();
                                            this.destination =
                                                format!("{}.{}", stem, this.format.extension());
                                        }
                                        cx.notify();
                                    }
                                },
                    ...
                    .child(
                        Button::new("add-folder")
                            ...
                            .label("Add Folder")
                            .on_click(cx.listener(
                                |this, _event: &ClickEvent, _window: &mut Window, cx| {
            ...
            .child(
                h_flex()
                    ...
                    .child(
                        Select::new(&self.level_select)
                            .title_prefix("Compression Level: ")
                            .disabled(!self.format.supports_compression_level()),
                    ),
            )
            .child(
                v_flex().gap_1().child(
                            ...
                            v_flex()
                                ...
                                .child(
                                    h_flex()
                                        ...
                                        .when(
                                            self.format == ArchiveFormat::SevenZip
                                                ...
                                                h_flex()
                                                    ...
                                                    .child(
                                                        div()
                                                            ...
                                                            .rounded_md()
                                                            .w(px(80.))
                                                            .child(if self.word_size.is_empty() {
                                                                "64".to_string()
                                                            } else {
                                                                self.word_size.clone()
                                                            }),
                                                    ),
                                            )
                                        }),
                                    ...
                                    h_flex()
                                        ...
                                        .child(
                                            div()
                                                ...
                                                .rounded_md()
                                                .w(px(100.))
                                                .child(if self.volume_size.is_empty() {
                                                    "\u{2014}".to_string()
                                                } else {
                                                    self.volume_size.clone()
                                                }),
                                        ),
                                )
                                .child(
            ...
            .child(
                v_flex()
                    ...
                    .child(Input::new(&self.password_state).mask_toggle().flex_1())
            ...
            .child(
                h_flex()
                    ...
                    .child(Button::new("cancel").label("Cancel").on_click(
                        cx.listener(|_this, _e, _window, cx| cx.emit(CreateDialogEvent::Canceled)),
                    ))
                    .child(
                        Button::new("create")
                            .label(if self.password.is_empty() {
                                "Create"
                            } else {
                                "Create Encrypted"
                            })
                            ...
                            .on_click(cx.listener(|this, _e, _window, cx| {
                                let service: Arc<ArchiveService> =
                                    bit7z_rt_app_state::AppState::global(cx).service.clone();
                                let dest = std::path::PathBuf::from(&this.destination);
                                let format = this.format;
                                let encryption = this.build_encryption();
                                let files: Vec<std::path::PathBuf> =
                                    this.file_list.iter().map(|f| f.path.clone()).collect();
                                let (tx, rx) = bit7z_infra_progress::progress_channel();
                                cx.update_global::<bit7z_pres_progress::ProgressState, _>(
                                    |state, _cx| {
                                ...
                                cx.background_spawn(async move {
                                    let mut handle =
                                        match service.create(&dest, format, encryption.as_ref()) {
                                            Ok(h) => h,
                                            Err(e) => {
                                                let _ = tx.send(
                                ...
                                cx.spawn(async move |_, cx| {
                                    loop {
                                        let done = cx
                                            .update_global::<bit7z_pres_progress::ProgressState, _>(
                                                |state, _| {
                                                    let _ = state.poll();
                                                    state.is_complete
                                                },
