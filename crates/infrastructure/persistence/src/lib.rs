...
impl ArchiveRepository for Bit7zRepository {
    ...
    fn get_properties(&self, archive: &ArchiveHandle) -> Result<ArchiveProperties, ArchiveError> {
        ...
        let volumes_count = unsafe { bit7z_ffi::bit7z_reader_volumes_count(raw as *mut _) };
        let headers_size = unsafe { bit7z_ffi::bit7z_reader_headers_size(raw as *mut _) };
        let has_comment = unsafe { bit7z_ffi::bit7z_reader_has_comment(raw as *mut _) != 0 };
        let dictionary_size = {
            let sz = unsafe { bit7z_ffi::bit7z_reader_dictionary_size(raw as *mut _) };
    ...
    fn extract_to_buffer(&self, archive: &ArchiveHandle, index: u32)
        ...
        let ret = unsafe {
            bit7z::bit7z_reader_extract_to_buffer_c(
                raw as *mut _,
                index,
                &mut out_data,
                &mut out_size,
    ...
    fn test(&self, archive: &ArchiveHandle) -> Result<TestResult, ArchiveError> {
        ...
        let count = unsafe { bit7z_ffi::bit7z_reader_item_count(raw as *mut _) };
        if count == 0 {
            return Ok(TestResult { total: 0, passed: 0, failed: vec![] });
        }

        let result = unsafe { bit7z_ffi::bit7z_reader_test(raw as *mut _) };
        if result.is_null() {
            return Err(ArchiveError::Internal(format!(
    ...
    fn list_directory(&self, archive: &ArchiveHandle, path: &str) -> Result<Vec<ArchiveEntry>, ArchiveError> {
        ...
        let list = unsafe { bit7z_ffi::bit7z_reader_list_directory(raw as *mut _, c_path.as_ptr()) };
        if list.is_null() {
            return Err(ArchiveError::NotFound(path.into()));
        }
        let count = unsafe { bit7z_ffi::bit7z_item_list_count(list) };
        let mut entries = Vec::with_capacity(count as usize);
        for i in 0..count {
            let path = unsafe {
                let p = bit7z_ffi::bit7z_item_list_path(list, i);
        ...
        unsafe { bit7z_ffi::bit7z_item_list_free(list); }
        Ok(entries)
    }
