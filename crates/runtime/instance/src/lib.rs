...
mod win {
    ...
    pub fn acquire(path: &Path) -> Result<super::InstanceLockWin, super::InstanceError> {
        ...
        let wide: Vec<u16> = OsStr::new(&key)
            .encode_wide()
        ...
        unsafe {
            ...
            if handle.is_null() {
                return Err(super::InstanceError::Internal("CreateMutex failed".into()));
            }
...
mod linux {
    ...
    pub fn acquire(path: &Path) -> Result<super::InstanceLockLinux, super::InstanceError> {
        ...
        match UnixListener::bind_addr(&addr) {
            Ok(l) => Ok(super::InstanceLockLinux {
                listener: Some(l),
                key,
            }),
